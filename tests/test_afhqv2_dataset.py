"""AFHQv2 filesystem loader tests (run in the repository Docker image)."""
...
class AFHQv2DatasetTest(unittest.TestCase):
  ...
  def test_identity_and_loader_use_prepared_pngs(self):
    with tempfile.TemporaryDirectory() as root:
      ...
      stats_filename = evaluation._get_stats_filename(config)
      self.assertRegex(
        stats_filename, r'^assets/stats/afhqv2_64_[0-9a-f]{12}_stats\.npz$')
  ...
  def test_reference_stats_use_every_prepared_image(self):
    with tempfile.TemporaryDirectory() as root:
      ...
      config = get_config()
      config.data.afhqv2_dir = root
      # Subset knobs are intentionally ignored for paper reference stats.
