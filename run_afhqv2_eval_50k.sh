...
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 {vp|ve|cosine_vp|edm|edm_canonical|fox_gaussian|fox_matern32|fox_matern32_vpdrift} [checkpoint] [config overrides...]" >&2
  exit 1
fi
...
case "${MODEL}" in
  ...
  ve)            CONFIG=configs/ve/afhqv2_ncsnpp_continuous.py ;;
