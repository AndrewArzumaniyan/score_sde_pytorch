...
def get_sde(config, n_override=None):
  ...
  if sde_name == 'foxvpsde':
    return _configure_sde_controls(sde_lib.FoxVPSDE(
      ...
      drift_k_bar=getattr(config.model, 'fox_drift_k_bar', -5.025),
      drift_a=getattr(config.model, 'fox_drift_a', 0.0),
      normalize_scale=getattr(config.model, 'fox_normalize_scale', False),
