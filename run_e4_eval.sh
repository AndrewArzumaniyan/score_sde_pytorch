...
# cudnn_benchmark / allow_tf32 from run_e4_common.sh (both default True to match
# the May-2026 reference checkpoints).  Extra `--config.*` -> forwarded to main.py.
...
echo "=== [E4 eval ${SUFFIX}] arm 1/3: VP  (ckpt-${E4_CKPT}) ==="
python -u main.py --config="${VP_CONFIG}" --workdir="${VP_WORKDIR}" "${COMMON_ARGS[@]}" "$@"

echo "=== [E4 eval ${SUFFIX}] arm 2/3: normalized-FOX  (ckpt-${E4_CKPT}) ==="
python -u main.py --config="${NORMFOX_CONFIG}" --workdir="${NORMFOX_WORKDIR}" "${COMMON_ARGS[@]}" "$@"

echo "=== [E4 eval ${SUFFIX}] arm 3/3: real-FOX kappa=1000  (ckpt-${E4_CKPT}) ==="
